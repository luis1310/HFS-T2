...
Después de encontrar la mejor combinación de operadores, se hace tunning de hiperparámetros usando experimentos sistemáticos que prueban distintas configuraciones de parámetros numéricos. Se prueban distintos valores para:
\begin{itemize}
...
Estos experimentos miden la calidad de las soluciones con varias métricas del frente de Pareto, lo que permite identificar configuraciones que den el mejor balance entre calidad, diversidad y convergencia.

\subsubsection{Optimizaciones algorítmicas iniciales}

Una vez identificada la mejor configuración comparando operadores y haciendo tunning, se implementaron optimizaciones algorítmicas iniciales para mejorar la eficiencia computacional. Estas optimizaciones incluyen caché de fitness, clasificación adaptativa y búsqueda local selectiva, que se detallan en la Sección \ref{sec:optimizaciones_algoritmicas}.

\subsubsection{Estudio de ablación}

Tras implementar las optimizaciones algorítmicas iniciales, se hizo un estudio de ablación sistemático para validar que cada componente del algoritmo es necesario y medir su impacto individual \cite{OptimalAblation2024}. El estudio de ablación es una metodología científica muy usada en IA, machine learning y algoritmos evolutivos. Determina la contribución de cada componente removiéndolo sistemáticamente y analizando cómo cambia el rendimiento \cite{AblationLLM2024, AblationAttentionModules2024}. Este estudio evalúa qué pasa cuando se desactivan componentes específicos, permitiendo cuantificar cuánto aporta cada elemento al rendimiento final.

\paragraph{Metodología del estudio de ablación}

El estudio de ablación se diseñó para evaluar sistemáticamente el impacto de cada componente comparando variantes que activan o desactivan componentes específicos \cite{AblationLLM2024}. Se probaron 6 variantes principales: NSGA-II completo (con filtro epsilon), NSGA-II sin filtro (base), Memético completo (con filtro epsilon, búsqueda local, cache y optimizaciones), Memético sin filtro, Memético sin búsqueda local, y Memético sin filtro ni búsqueda. Cada variante se corrió con 30 semillas independientes para garantizar robustez estadística, dando un total de 180 ejecuciones.

Los resultados del estudio de ablación permiten varias cosas: cuantificar el impacto de cada componente en tiempo y calidad, justificar científicamente su inclusión, identificar componentes críticos versus opcionales, y comparar la eficiencia entre NSGA-II estándar y memético. Los resultados detallados y el análisis cuantitativo están en el Capítulo 4.

\paragraph{Optimizaciones basadas en ablación: vectorización con NumPy}

Los resultados del estudio de ablación mostraron que las operaciones más costosas del algoritmo se podían optimizar con vectorización. Se identificaron tres operaciones críticas que consumían 40-50\% del tiempo total. La Clasificación No Dominada tomaba 20-30\% con operaciones O(n²) usando bucles anidados en Python puro. El Filtro Epsilon tomaba 10-15\% con muchas conversiones y cálculos repetitivos. La Distancia Crowding tomaba 5-10\% con ordenamientos repetitivos costosos.

Para optimizar estas operaciones se usó vectorización con NumPy en \texttt{tesis3/src/algorithms/nsga2.py}. Se reemplazaron bucles anidados y list comprehensions por operaciones vectorizadas que aprovechan broadcasting e indexación avanzada. Las tres funciones optimizadas son: \texttt{clasificacion\_no\_dominada}, \texttt{filtrar\_soluciones\_similares} y \texttt{distancia\_crowding}.

Es importante recalcar que estas optimizaciones de vectorización trabajan exclusivamente sobre los valores de \textit{fitness} (objetivos a maximizar) y \textbf{no modifican la estructura del cromosoma}. El cromosoma mantiene su representación original como array de enteros que indica las asignaciones de máquinas por etapa para cada pedido. La vectorización solo acelera las operaciones de clasificación, filtrado y cálculo de distancias que procesan los valores de fitness, sin alterar la representación genética de las soluciones. Los resultados cuantitativos de estas optimizaciones se presentan en el Capítulo 4.
