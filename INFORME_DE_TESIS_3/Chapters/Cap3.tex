...
Para optimizar estas operaciones, se implementó una estrategia de vectorización utilizando NumPy en el archivo \texttt{tesis3/src/algorithms/nsga2.py}:

\begin{itemize}
    \item \textbf{Vectorización de Clasificación No Dominada} (función \texttt{clasificacion\_no\_dominada}): Conversión de bucles anidados O(n²) a operaciones vectorizadas usando broadcasting de NumPy. Se convierte \texttt{fitness\_poblacion} a un array NumPy y se utilizan operaciones de broadcasting (\texttt{fitness\_array[:, None, :]} y \texttt{fitness\_array[None, :, :]}) para calcular la matriz de dominancia \texttt{i\_domina\_j} comparando todos los pares simultáneamente, reemplazando los bucles anidados en Python.
    
    \item \textbf{Vectorización del Filtro Epsilon} (función \texttt{filtrar\_soluciones\_similares}): Reemplazo de list comprehensions por operaciones vectorizadas con arrays NumPy. Se vectoriza la conversión de fitness a métricas (\texttt{makespans = 1 / fitness\_array[:, 0]}, etc.), se utiliza \texttt{np.ptp} para cálculo de rangos, y se vectoriza la normalización y discretización para asignación a grilla espacial, reduciendo la complejidad de comparaciones.
    
    \item \textbf{Vectorización de Distancia Crowding} (función \texttt{distancia\_crowding}): Reemplazo de \texttt{sorted} con key function por \texttt{np.argsort} para obtener índices ordenados, y vectorización de cálculos de distancias para elementos intermedios usando indexación avanzada de NumPy.
\end{itemize}
...
Estas optimizaciones lograron una reducción del 56.7\% en el tiempo de ejecución (de 26.55s a 11.5s promedio, estabilizado en ~10.7s después del warm-up), mejorando significativamente la eficiencia computacional mientras mantienen la calidad del frente de Pareto sin afectar la funcionalidad del algoritmo.
