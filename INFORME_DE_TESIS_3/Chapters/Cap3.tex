...
El problema clásico del flow shop hibrido flexible (HFS, por sus siglas en inglés) supone un conjunto de $P$ pedidos que deben ser procesados en $K$ etapas. Cada etapa contiene varias máquinas paralelas idénticas y la ruta de proceso es igual para cada pedido; teniendo en cuenta un conjunto de suposiciones estándar \cite{Song202156822}, el objetivo es encontrar la programación que logre el mínimo makespan. El tiempo que transcurre entre el inicio del primer pedido en la primera máquina y la terminación del último pedido en la última máquina se conoce con el término de makespan \cite{Garavito-Hernández2019362}. Se considera que minimizar el makespan equivale a maximizar la utilización de las máquinas, ya que se busca disminuir los tiempos de alistamiento y los tiempos de ocio de las máquinas.
...
Una representación usual en la literatura es la de una representación binaria \cite{Lop_et_al_2015}, tomandolo como un array de desiciones, tal como se ve en la figura \ref{crom_bin}, sin embargo, para una cuantiosa cantidad de pedidos, maquinas y etapas esto no es viable, en caso se requiera el cromosoma para 3 pedidos y una distribución de maquinas como la mencionada en la figura \ref{maquinas} se usa un array de 33 genes, los cuales tendrían valores de 0 o 1.
...
El algoritmo Non-dominated Sorting Genetic Algorithm II (NSGA-II) \textcite{Deb2002} es una metaheurística evolutiva multiobjetivo desarrollada en 2002. Este algoritmo está diseñado para resolver problemas de optimización con múltiples objetivos simultáneos mediante el concepto de dominancia de Pareto.
...
\begin{itemize}
    ...
    \item \( E_k \) es la \( k \)-ésima etapa de producción (donde \( k = 1, 2, \dots, 5 \)).
    ...
    \item \( n \) es el número de pedidos que se solicitarán.
...
Este método realiza un cruce uniforme considerando la estructura de etapas del problema \cite{CooperativeGAHFS2023}. Para cada etapa de cada pedido, se decide aleatoriamente (con probabilidad 0.5) si la asignación de máquina proviene del padre 1 o del padre 2. De esta forma, cada gen (asignación de máquina) se hereda independientemente de uno de los padres. Este operador preserva la validez de los cromosomas asegurando que cada pedido mantenga una máquina válida para cada etapa. Este operador es particularmente efectivo en problemas multiobjetivo ya que combina soluciones de manera uniforme, explorando eficientemente el espacio de compromiso entre objetivos.

\subsubsection{Cruzamiento de Un Punto}
En este método se selecciona un punto de corte aleatorio entre pedidos \cite{AutomaticAlgorithmDesignHFS2023}. Para cada pedido, si su posición es anterior al punto de corte, mantiene las asignaciones del padre 1; si es posterior al punto de corte, hereda las asignaciones del padre 2. El hijo 2 se genera con la lógica inversa. Este operador proporciona una variación más conservadora del genoma, agrupando pedidos completos y manteniendo bloques de asignaciones de máquinas de cada padre, útil cuando el frente de Pareto está convergiendo \cite{HybridFlowShopMultiCrossover2023, SustainableHFSScheduling2023}.

\subsection{Métodos de Mutación}

Se implementan tres métodos de mutación orientados a la estructura de etapas del problema \cite{GeneticOperatorsReview2024, EnhancingMOEAScheduling2024}. Todos estos operadores respetan las restricciones del problema, asegurando que cada etapa reciba una máquina válida. Los operadores de mutación swap, insert e invert han demostrado ser efectivos en problemas de scheduling mediante algoritmos evolutivos \cite{ViralMutationFlowshop2024, RealTimeSchedulingGA2024}.

\subsubsection{Mutación Swap Stage-Aware}
Este método intercambia las máquinas asignadas entre dos pedidos en la misma etapa. Para una etapa seleccionada aleatoriamente, se eligen dos pedidos distintos y se intercambian sus asignaciones de máquinas en esa etapa. Este operador es efectivo para redistribuir cargas entre pedidos manteniendo la estructura general del cromosoma. El operador \textit{swap} es uno de los operadores de mutación más estudiados en problemas combinatoriales de scheduling \cite{GeneticOperatorsReview2024}, y se ha demostrado que es particularmente efectivo cuando se combina con otros operadores para resolver problemas de optimización en sistemas de tiempo real \cite{RealTimeSchedulingGA2024}.

\subsubsection{Mutación Insert Stage-Aware}
Este método modifica la asignación de máquina de un pedido en una etapa específica. Se selecciona aleatoriamente un pedido y una etapa, y se cambia la máquina asignada en esa posición por otra máquina válida para esa etapa (diferente a la actual). Este operador proporciona diversidad local al permitir explorar diferentes opciones de máquinas para el mismo pedido en cada etapa. La mutación de inserción es especialmente útil en problemas donde el orden y la asignación de recursos son importantes \cite{ViralMutationFlowshop2024}.

\subsubsection{Mutación Invert Stage-Aware}
Este método invierte la secuencia de asignaciones de máquinas de un pedido en un rango de etapas consecutivas. Se selecciona aleatoriamente un pedido y dos etapas, y se invierte la secuencia de máquinas asignadas entre esas etapas. Este operador preserva las asignaciones individuales pero altera su orden, introduciendo variación en la estructura del cromosoma. La mutación de inversión se ha mostrado efectiva en problemas de ruteo y scheduling donde el orden de operaciones es crítico \cite{EnhancingMOEAScheduling2024}.

\subsection{Metodología de Selección de Configuración}

Para determinar la configuración óptima de los algoritmos implementados (NSGA-II y NSGA-II memético), se emplea una metodología sistemática que combina la comparación de operadores, el tunning de hiperparámetros, la implementación de optimizaciones algorítmicas iniciales, y finalmente el estudio de ablación para identificar mejoras adicionales. Esta metodología permite identificar la configuración más adecuada que se ajuste específicamente al problema de programación en líneas de ensamblaje.

\subsubsection{Comparación de Operadores}
En primer lugar, se realizan experimentos de comparación sistemática entre los diferentes operadores implementados:
\begin{itemize}
...
Estos experimentos determinan qué combinaciones de operadores proporcionan mejores resultados en términos de calidad del frente de Pareto, diversidad de soluciones y robustez de los algoritmos. La mejor combinación identificada se utiliza como base para las siguientes fases de optimización.

\subsubsection{Tunning Multiobjetivo}
Una vez identificada la mejor combinación de operadores, se realiza el tunning de hiperparámetros mediante experimentos sistemáticos que evalúan múltiples configuraciones de parámetros numéricos. Se consideran diferentes valores para:
\begin{itemize}
...
Después de identificar la mejor configuración mediante comparación de operadores y tunning, se implementaron optimizaciones algorítmicas iniciales para mejorar la eficiencia computacional. Estas optimizaciones incluyen caché de fitness, clasificación adaptativa y búsqueda local selectiva, las cuales se describen en detalle en la Sección \ref{sec:optimizaciones_algoritmicas}.

\subsubsection{Estudio de Ablación}

Una vez implementadas las optimizaciones algorítmicas iniciales, se realizó un estudio de ablación sistemático para validar científicamente que cada componente del algoritmo es necesario y medir su impacto individual \cite{OptimalAblation2024}. El estudio de ablación es una metodología científica ampliamente utilizada en inteligencia artificial, machine learning y algoritmos evolutivos para determinar la contribución individual de cada componente de un sistema mediante su remoción sistemática y el análisis del rendimiento resultante \cite{AblationLLM2024, AblationEvolutionaryAlgorithms2024, AblationAttentionModules2024}. Este estudio evalúa el impacto de desactivar componentes específicos del algoritmo, permitiendo cuantificar la contribución de cada elemento al rendimiento final.

\paragraph{Metodología del Estudio de Ablación}

El estudio de ablación se diseñó para evaluar sistemáticamente el impacto de cada componente del algoritmo mediante la comparación de variantes que activan o desactivan componentes específicos \cite{AblationLLM2024}. Se evaluaron 6 variantes principales del algoritmo: NSGA-II completo (con filtro epsilon), NSGA-II sin filtro (base), Memético completo (con filtro epsilon, búsqueda local, cache y optimizaciones), Memético sin filtro, Memético sin búsqueda local, y Memético sin filtro ni búsqueda. Cada variante se ejecutó con 30 semillas independientes para garantizar robustez estadística, resultando en un total de 180 ejecuciones.

Los resultados del estudio de ablación permiten cuantificar el impacto de cada componente en el tiempo de ejecución y la calidad de las soluciones, justificar científicamente la inclusión de cada componente en el diseño final del algoritmo, identificar componentes críticos que deben mantenerse y componentes opcionales que pueden optimizarse, y comparar la eficiencia relativa entre NSGA-II estándar y memético. Los resultados detallados y análisis cuantitativo se presentan en el Capítulo 4.

\paragraph{Optimizaciones Basadas en Ablación: Vectorización con NumPy}

Los resultados del estudio de ablación revelaron que las operaciones más costosas del algoritmo eran susceptibles de optimización mediante vectorización. Se identificaron tres operaciones críticas que representaban aproximadamente el 40-50\% del tiempo total de ejecución: (1) Clasificación No Dominada (20-30\% del tiempo), con operaciones O(n²) con bucles anidados en Python puro; (2) Filtro Epsilon (10-15\% del tiempo), con múltiples conversiones y cálculos repetitivos; y (3) Distancia Crowding (5-10\% del tiempo), con ordenamientos repetitivos costosos.

Para optimizar estas operaciones, se implementó una estrategia de vectorización utilizando NumPy en el archivo \texttt{tesis3/src/algorithms/nsga2.py}, reemplazando bucles anidados y list comprehensions por operaciones vectorizadas que aprovechan el broadcasting y la indexación avanzada de NumPy. Las tres funciones optimizadas son: \texttt{clasificacion\_no\_dominada}, \texttt{filtrar\_soluciones\_similares} y \texttt{distancia\_crowding}.

\textbf{Importante}: Estas optimizaciones de vectorización trabajan exclusivamente sobre los valores de \textit{fitness} (objetivos a maximizar) y \textbf{no modifican la estructura del cromosoma}. El cromosoma mantiene su representación original como array de enteros que indica las asignaciones de máquinas por etapa para cada pedido. La vectorización solo acelera las operaciones de clasificación, filtrado y cálculo de distancias que procesan los valores de fitness, sin alterar la representación genética de las soluciones. Los resultados cuantitativos de estas optimizaciones se presentan en el Capítulo 4.
...
Este enfoque es estándar en optimización multiobjetivo: \textcite{Deb2002} emplean normalización basada en límites del espacio objetivo sin requerir calibración empírica precisa; estudios recientes \cite{NormalizationPBEMO2024, HypervolumeNormalization2022} han investigado diferentes métodos de normalización en algoritmos evolutivos multiobjetivo, demostrando que los valores de referencia para métricas de calidad (como hipervolumen) afectan magnitudes pero no rankings relativos. La práctica común en la literatura es establecer valores de referencia que sean "razonables" para el problema, no necesariamente "óptimos" u obtenidos empíricamente.
...
\begin{itemize}
    ...
    \item $C_{eval}$: costo de una evaluación de \textit{fitness} para un individuo (depende de la instancia del problema; en general incluye calcular tiempo de proceso, enfriamientos y energía sobre todas las etapas).\footnote{En la práctica, $C_{eval}$ escala con el tamaño de la instancia: número de pedidos, etapas y máquinas por etapa.}
...
\begin{enumerate}
    \item \textbf{Evaluación de la población}: $\mathcal{O}(N\,C_{eval})$.
...
\[
\mathcal{O}\big(M\,N^{2} + M\,N\log N + N\,C_{eval} + N\,L\big) = \mathcal{O}\big(M\,N^{2} + N\,(C_{eval}+L)\big),
\]
siendo el término $\mathcal{O}(M\,N^{2})$ típicamente dominante.

En $G$ generaciones, la complejidad temporal total es:
\[
\mathcal{T}_{\text{NSGA-II}}(N,G) = \mathcal{O}\big(G\,M\,N^{2} + G\,N\,(C_{eval}+L)\big) = \mathcal{O}\big(G\,M\,N^{2}\big).
\]

\paragraph{Coste adicional por la búsqueda local (versión memética)}

La variante memética aplica una búsqueda local cada $k$ generaciones, hasta $I$ iteraciones por individuo seleccionado. Si se aplica sobre la población completa (caso peor), y el vecindario evalúa $\Theta(1)$ movimientos por iteración (p.ej., \textit{swap/insert/invert} con evaluación incremental), el coste adicional por ciclo de búsqueda local es $\mathcal{O}(N\,I\,C_{eval})$. Dado que ocurre aproximadamente $G/k$ veces:
\[
\mathcal{T}_{\text{memético}}^{\text{local}} = \mathcal{O}\Big(\frac{G}{k}\,N\,I\,C_{eval}\Big).
\]

Así, la complejidad total del NSGA-II memético implementado es:
\[
\mathcal{T}_{\text{NSGA-II-mem}}(N,G) = \mathcal{O}\big(G\,M\,N^{2}\big)\; +\; \mathcal{O}\Big(\frac{G}{k}\,N\,I\,C_{eval}\Big)\; +\; \mathcal{O}\big(G\,N\,(C_{eval}+L)\big).
\]
En la práctica, el término $\mathcal{O}(G\,M\,N^{2})$ sigue siendo dominante salvo que $\tfrac{I\,C_{eval}}{k}$ sea grande. Esto justifica, en los experimentos, limitar $I$ y espaciar la búsqueda local con $k$ para mantener tiempos manejables.
...
En nuestros experimentos, con $M=3$ objetivos y los rangos $N\in\{100,200\}$, $G\in\{400,600\}$, la cota dominante $\mathcal{O}(G\,M\,N^{2})$ explica el tiempo de cómputo observado y la importancia de la paralelización por semillas. La búsqueda local (parámetros \texttt{cada\_k\_gen} y \texttt{max\_iter\_local}) introduce un término adicional proporcional a $\tfrac{G}{k}\,N\,I\,C_{eval}$, por lo que se recomienda (y así se hizo) mantener $I$ y $k$ en valores moderados para equilibrar calidad/tiempo.

\subsection{Optimizaciones Implementadas en el Algoritmo}
\label{sec:optimizaciones_algoritmicas}

Para mejorar la eficiencia computacional y la calidad del frente de Pareto, se implementaron varias optimizaciones algorítmicas iniciales en el algoritmo NSGA-II memético después de la fase de tunning:
...
Con la metodología establecida, los algoritmos implementados y las optimizaciones aplicadas, el siguiente capítulo presenta los resultados experimentales obtenidos mediante la aplicación sistemática de esta metodología, incluyendo el análisis detallado del frente de Pareto y la comparación con trabajos previos en la literatura.
