...
Una vez implementadas las optimizaciones algorítmicas iniciales, se realizó un estudio de ablación sistemático para validar científicamente que cada componente del algoritmo es necesario y medir su impacto individual \cite{AblationAlgorithmConfig2014}. El estudio de ablación es una metodología científica ampliamente utilizada en inteligencia artificial y algoritmos evolutivos para determinar la contribución individual de cada componente de un sistema mediante su remoción sistemática y el análisis del rendimiento resultante \cite{AblationNeuralNetworks2019, AblationEvolutionaryAlgorithms2024}. Este estudio evalúa el impacto de desactivar componentes específicos del algoritmo, permitiendo cuantificar la contribución de cada elemento al rendimiento final.

\paragraph{Metodología del Estudio de Ablación}

El estudio de ablación se diseñó para evaluar sistemáticamente el impacto de cada componente del algoritmo mediante la comparación de variantes que activan o desactivan componentes específicos \cite{AblationAlgorithmConfig2014}. Se evaluaron 6 variantes principales del algoritmo: NSGA-II completo (con filtro epsilon), NSGA-II sin filtro (base), Memético completo (con filtro epsilon, búsqueda local, cache y optimizaciones), Memético sin filtro, Memético sin búsqueda local, y Memético sin filtro ni búsqueda. Cada variante se ejecutó con 30 semillas independientes para garantizar robustez estadística, resultando en un total de 180 ejecuciones.
