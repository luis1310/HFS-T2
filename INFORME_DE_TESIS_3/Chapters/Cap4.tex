...
Los experimentos se realizaron usando el Mostrito el cual contó con 32 núcleos físicos, 32 núcleos lógicos y 120 gb de RAM, haciendo uso de la paralelización. Se usaron 32 núcleos lógicos para maximizar el rendimiento.
...
Esta combinación demostró ser superior en términos de balance entre los tres objetivos, mostrando una buena convergencia y diversidad en el frente de Pareto. Esta configuración se utilizó como base para las siguientes fases de optimización.
...
\subsection{Optimizaciones algorítmicas iniciales}

Después de identificar la mejor configuración mediante comparación de operadores y tunning, se implementaron optimizaciones algorítmicas iniciales que lograron reducir el tiempo de ejecución de ~60 segundos a ~27 segundos:
...
Basándose en los resultados del estudio de ablación, se implementaron optimizaciones de vectorización con NumPy en tres funciones críticas del algoritmo (metodología detallada en el Capítulo 3, Sección 3.3.4): \texttt{clasificacion\_no\_dominada}, \texttt{filtrar\_soluciones\_similares} y \texttt{distancia\_crowding}. Estas optimizaciones trabajan exclusivamente sobre los valores de \textit{fitness} y no modifican la estructura del cromosoma.

\paragraph{Resultados de las optimizaciones de vectorización}
...
El estudio de ablación identificó que las operaciones más costosas (clasificación no dominada, filtro epsilon, distancia crowding) eran susceptibles de optimización mediante vectorización con NumPy. Las optimizaciones implementadas (descritas metodológicamente en el Capítulo 3) lograron una reducción del 56.7\% en el tiempo de ejecución (de 26.55s a 11.5s promedio, estabilizado en ~10.7s), manteniendo la calidad del frente de Pareto con 51-82 soluciones únicas.
...
Habiendo presentado y discutido los resultados experimentales, el siguiente capítulo sintetiza las conclusiones principales del trabajo, valida las hipótesis planteadas en la introducción y propone direcciones futuras de investigación basadas en los hallazgos obtenidos.
