...
La mejor configuración identificada mediante el score agregado (promedio sobre 30 semillas) fue:

\begin{itemize}
    \item \textbf{Tamaño de población}: 150 individuos
    \item \textbf{Número de generaciones}: 600 generaciones
    \item \textbf{Probabilidad de cruce}: 0.7
...
Esta configuración obtuvo un score agregado promedio de 2.5981, con las siguientes métricas promedio:
\begin{itemize}
    \item Makespan: 1552.06 minutos
    \item Balance: 279.06
    \item Energía: 624.31 kWh
    \item Tiempo de ejecución: 73.88 segundos
    \item Tamaño del frente de Pareto: 149.73 soluciones
\end{itemize}
...
Utilizando la mejor configuración identificada (NSGA-II memético con cruce uniforme y mutación swap), se generó un frente de Pareto final con 78 soluciones únicas después de aplicar el filtro de similitud ($\epsilon = 1\%$).
...
\begin{itemize}
    \item \textbf{Rango de makespan}: 1541.92 - 1545.05 minutos (rango de 2.76 minutos)
    \item \textbf{Rango de balance}: 261.03 - 329.93 (rango de 68.90 unidades)
    \item \textbf{Rango de energía}: 621.26 - 627.37 kWh (rango de 6.11 kWh)
\end{itemize}
...
\begin{itemize}
    \item Si se prioriza la \textbf{productividad temporal}, se puede elegir una solución con makespan mínimo (1541.92 minutos), aunque esto puede resultar en mayor desbalance de carga o consumo energético.
    \item Si se prioriza la \textbf{equidad operativa}, se puede elegir una solución con balance mínimo (261.03), aunque esto puede extender el makespan o aumentar el consumo energético.
    \item Si se prioriza la \textbf{sostenibilidad energética}, se puede elegir una solución con consumo energético mínimo (621.26 kWh), aunque esto puede afectar el makespan o el balance de carga.
    \item Si se busca un \textbf{compromiso equilibrado}, se puede elegir una solución intermedia que balancee los tres objetivos de manera razonable.
\end{itemize}
...
\caption{Evolución del tamaño del frente de Pareto a lo largo de 600 generaciones}
...
\begin{itemize}
    \item \textbf{Tiempo promedio por ejecución}: 25-30 segundos (con la mejor configuración)
    \item \textbf{Reducción respecto a versión inicial}: Aproximadamente 60\% de reducción (de ~60 segundos a ~25 segundos)
