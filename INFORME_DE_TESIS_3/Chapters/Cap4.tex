...
Basándose en los resultados del estudio de ablación, se identificaron las operaciones más costosas del algoritmo que eran susceptibles de optimización mediante vectorización. Se implementaron optimizaciones de vectorización con NumPy en tres funciones críticas del algoritmo: \texttt{clasificacion\_no\_dominada}, \texttt{filtrar\_soluciones\_similares} y \texttt{distancia\_crowding}. Estas optimizaciones trabajan exclusivamente sobre los valores de \textit{fitness} y no modifican la estructura del cromosoma.
...
Utilizando la mejor configuración identificada (NSGA-II estándar con cruce uniforme y mutación swap) y después de aplicar todas las optimizaciones (algorítmicas iniciales y vectorización), se generó un frente de Pareto final con 51-82 soluciones únicas después de aplicar el filtro de similitud ($\epsilon = 1\%$).
