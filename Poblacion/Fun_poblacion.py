...
def inicializar_poblacion(tamano_poblacion=tamano_poblacion, maquinas_por_etapa=maquinas_por_etapa, num_pedidos=num_pedidos):
    poblacion = []
    individuos_existentes = set()

    while len(poblacion) < tamano_poblacion:
        # Genera un nuevo individuo
        individuo = [[random.choice(maquinas_por_etapa[i]) for i in range(5)] for _ in range(num_pedidos)]
